---
description: >-
  This section introduces the integration of sendTx(Send in Plain Text) provided
  by BlockRazor Robinhood Chain RPC
---

# Send in Plain Text

`Send in Plain Text` is used to send signed transaction on Robinhood. It presents a much more streamlined and rapid method for submitting transactions compared with&#x20;
...
{% tab title="HTTPS" %}
```bash
curl -X POST 'https://robinhood.blockrazor.io/v2/sendTx?auth=<auth_token>' \
...
* **the `auth` and `method` are compulsory to be added in URI params, e.g.,** `https://robinhood.blockrazor.io/v2/sendTx?auth=<auth_token>`
