...
{% hint style="info" %}
[fast mode](https://github.com/BlockRazorinc/solana-trader-client-rust/blob/main/src/mode_grpc_fast.rs)\
[sandwichMitigation mode](https://github.com/BlockRazorinc/solana-trader-client-rust/blob/main/src/mode_grpc_sandwichMitigation.rs)
{% endhint %}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // safe window
    let safe_window = None; // only take effect in sandwichMitigation mod
    // revert protection
    let revert_protection = false;
    // tip amount
